...
class ProgramController extends AdminController
{
    ...
    protected function form()
    {
        ...
        if(Admin::user()->can('*'))
            $device_auth = Device::WHERE("deviceinfo.status",1)->PLUCK('name', 'deviceCode');
        else
            $device_auth = Device::WHERE("deviceinfo.status",1)->WHEREIN('areaId',explode(',',Admin::user()->areaId))->PLUCK('name', 'deviceCode');
